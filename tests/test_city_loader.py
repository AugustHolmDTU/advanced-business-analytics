...
class TomTomSnapshotCityLoaderTest(unittest.TestCase):
    def test_build_city_from_tomtom_snapshot(self) -> None:
        snapshot_payload = {
            ...
            "stations": [
                {"name": "A", "address": "Addr A", "total_connectors": 8, "available_connectors": 4, "max_power_kw": 150.0},
                {"name": "B", "address": "Addr B", "total_connectors": 2, "available_connectors": 1, "max_power_kw": 50.0},
            ],
        }
        env_config = {
        ...
        self.assertEqual(city.layout_type, "tomtom_snapshot")
        ...
        self.assertEqual(city.site_labels, ("A", "B"))
        self.assertEqual(city.site_types[0], "major_hub")
        self.assertTrue(np.all(city.zone_base_demand >= 2.0))
        self.assertTrue(np.all(city.site_latlon.shape == np.asarray([[55.679, 12.533], [55.677, 12.541]], dtype=np.float32).shape))
