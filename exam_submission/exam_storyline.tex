...
Current checkpoint status & No current compatible checkpoint found in
checkout \\
\end{longtable}
...
the cost weight is too low, the controller may over-deploy MCS. If the
queue penalty is too low, the controller may accept poor service rather
than spend mobile capacity.

The historical reward sweep is useful here because it shows how changing
these weights changes the learned operating point.
...
\bottomrule\noalign{}
\endlastfoot
range\_c18\_q2p5 & 18.0 & 2.5 & 0.62 & 1.57 & 2.08 & -102.93 \\
range\_c24\_q2p5 & 24.0 & 2.5 & 0.12 & 0.5 & 2.17 & -146.77 \\
range\_c30\_q3 & 30.0 & 3.0 & 2.25 & 5.58 & 2.46 & -232.35 \\
range\_c30\_q4 & 30.0 & 4.0 & 2.0 & 6.74 & 1.79 & -199.99 \\
range\_c36\_q3 & 36.0 & 3.0 & 3.12 & 7.12 & 2.5 & -309.23 \\
range\_c42\_q3p5 & 42.0 & 3.5 & 5.42 & 10.31 & 1.79 & -305.46 \\
\end{longtable}

    
    The trade-off is clear. Higher MCS cost tends to push the policy toward
lower MCS usage, but that usually comes with worse queue outcomes.
Increasing queue penalties pushes the controller toward protecting
service quality more aggressively. In other words, there is no single
universally correct reward: the preferred policy depends on how
expensive mobile support is assumed to be relative to poor queue
performance.
...
\end{longtable}
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{exam_storyline_files/exam_storyline_25_2.png}
    \end{center}
    { \hspace*{\fill} \\}
...
story clean.
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{exam_storyline_files/exam_storyline_29_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
demand, local queue length, and MCS allocation for AB and BC separately.
    ...
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{exam_storyline_files/exam_storyline_35_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\end{longtable}
    ...
    \subsubsection{B. Additional policy comparison including the reactive
rule}\label{b.-additional-policy-comparison-including-the-reactive-rule}

    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{exam_storyline_files/exam_storyline_43_0.png}
    \end{center}
    { \hspace*{\fill} \\}
...
\bottomrule\noalign{}
\endlastfoot
Fixed / no-agent & 16.6 & 12.37 & 0.0 & -15969.37 & 0.0 & 0.0 \\
Adaptive threshold & 2.65 & 1.28 & 0.95 & -4202.84 & 84.06 & 89.62 \\
Reactive & 1.29 & 0.71 & 8.98 & -24060.17 & 92.24 & 94.25 \\
\end{longtable}

    
    The reactive rule is useful as a secondary benchmark because it drives
queues down even more aggressively than the threshold controller.
However, it does so by keeping many more mobile units active and is
therefore less suitable as the clean main comparison when cost
sensitivity matters.

    \subsubsection{C. Reduced multi-seed and stress-suite
evidence}\label{c.-reduced-multi-seed-and-stress-suite-evidence}

    \begin{longtable}[]{@{}lllll@{}}
\toprule\noalign{}
Policy & Mean queue & Mean wait (min) & Mean active MCS & Reward \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Fixed / no-agent & 17.74 & 13.43 & 0.0 & -13156.33 \\
Reactive & 2.21 & 1.08 & 7.98 & -17240.75 \\
Adaptive threshold & 3.65 & 1.82 & 1.09 & -4155.96 \\
\end{longtable}

    
    \begin{center}
    \adjustimage{max size={0.9\linewidth}{0.9\paperheight}}{exam_storyline_files/exam_storyline_46_1.png}
    \end{center}
    { \hspace*{\fill} \\}
    
    The reduced multi-seed evidence supports the same qualitative message as
the single held-out rollout: fixed capacity is fragile under disruption,
while adaptive mobile support improves service outcomes. The
stress-suite heatmaps also show that performance differences persist
across multiple named disruption patterns rather than depending on only
one favorable scenario.

    \subsubsection{D. Reference code and artifact
inventory}\label{d.-reference-code-and-artifact-inventory}
...
\endlastfoot
README.md & Config \\
configs/debug/logging\_disabled.yaml & Config \\
configs/demand/base.yaml & Config \\
configs/demo/mobile\_mcs\_line\_abc\_demo\_short.yaml & Config \\
configs/env/mobile\_mcs\_line\_abc.yaml & Config \\
configs/experiment/mobile\_mcs\_line\_abc.yaml & Config \\
configs/experiment/mobile\_mcs\_line\_abc\_heldout\_comparison.yaml &
Config \\
configs/experiment/mobile\_mcs\_line\_abc\_train\_only.yaml & Config \\
configs/rl/dqn\_mobile\_sb3.yaml & Config \\
configs/rl/dqn\_mobile\_simple.yaml & Config \\
src/evch/\textbf{init}.py & Code \\
src/evch/baselines/\textbf{init}.py & Code \\
src/evch/baselines/policies.py & Code \\
src/evch/config/\textbf{init}.py & Code \\
src/evch/config/loader.py & Code \\
src/evch/data/\textbf{init}.py & Code \\
src/evch/data/city.py & Code \\
src/evch/data/features.py & Code \\
src/evch/data/synthetic.py & Code \\
src/evch/envs/\textbf{init}.py & Code \\
src/evch/envs/charging\_env.py & Code \\
src/evch/envs/corridor\_mobile\_env.py & Code \\
src/evch/envs/demand.py & Code \\
src/evch/envs/factory.py & Code \\
src/evch/envs/line\_corridor\_mobile\_env.py & Code \\
src/evch/envs/mobile\_station\_env.py & Code \\
src/evch/models/\textbf{init}.py & Code \\
src/evch/models/common.py & Code \\
src/evch/rl/\textbf{init}.py & Code \\
src/evch/rl/evaluation.py & Code \\
src/evch/rl/simple\_dql.py & Code \\
src/evch/rl/simple\_dqn.py & Code \\
src/evch/sim/\textbf{init}.py & Code \\
src/evch/sim/line\_corridor.py & Code \\
src/evch/sim/simple\_corridor.py & Code \\
src/evch/train/\textbf{init}.py & Code \\
src/evch/train/eval\_suites.py & Code \\
src/evch/train/evaluate\_policies.py & Code \\
src/evch/train/run\_mobile\_rl\_comparison.py & Code \\
src/evch/train/train\_rl.py & Code \\
src/evch/utils/\textbf{init}.py & Code \\
src/evch/utils/io.py & Code \\
src/evch/utils/logging.py & Code \\
src/evch/utils/seeding.py & Code \\
src/evch/utils/torch\_runtime.py & Code \\
src/evch/utils/wandb.py & Code \\
\end{longtable}
