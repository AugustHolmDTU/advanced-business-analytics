...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Resilience under disruptions:} Can adaptive deployment of
  mobile charging stations reduce queue length and waiting time when the
  corridor is exposed to disruptions?
\item
...
normal day, but whether adaptive control can help the charging network
keep serving vehicles when conditions get worse locally.
...
The agent stores past transitions in a replay buffer:

\[
(s, a, r, s', \text{done})
\]

Training batches are sampled from this buffer. This makes learning more
stable because the model does not only learn from the most recent step,
but from a mix of earlier experiences.
...
to learn a very complex black-box policy. The goal is to test whether an
adaptive controller can learn useful deployment behavior in a disrupted
corridor.
...
\item
  runs one held-out \texttt{test\_id} rollout for the trained RL agent.
\end{itemize}
...
one learning-curve figure and one held-out test figure.
